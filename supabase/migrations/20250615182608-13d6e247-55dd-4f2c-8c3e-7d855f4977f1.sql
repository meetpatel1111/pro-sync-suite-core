...
-- The problem is likely in the validate_task_assignment function where we're comparing UUIDs
...
BEGIN
  ...
  IF NEW.assignee_id IS NOT NULL AND NEW.assigned_to IS NOT NULL THEN
    ...
    IF NOT (NEW.assignee_id::text = ANY(SELECT unnest(NEW.assigned_to)::text)) THEN
